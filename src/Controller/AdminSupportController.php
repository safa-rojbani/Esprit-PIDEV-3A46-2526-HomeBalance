...
use App\Service\Ai\SupportReplyService;
use App\Service\Ai\SupportTicketClassificationService;
...
final class AdminSupportController extends AbstractController
{
    public function __construct(
        private readonly SupportTicketClassificationService $classificationService,
        private readonly SupportReplyService $supportReplyService,
    ) {
    }
    ...
    #[Route('/{id}/ai/classify', name: 'ai_classify', methods: ['POST'], requirements: ['id' => '\d+'])]
    public function aiClassify(SupportTicket $ticket): JsonResponse
    {
        $this->denyAccessUnlessGranted('ROLE_ADMIN');

        $result = $this->classificationService->classify($ticket);
    ...
    public function aiSuggestReplies(SupportTicket $ticket): JsonResponse
    {
        $this->denyAccessUnlessGranted('ROLE_ADMIN');

        $suggestions = $this->supportReplyService->suggestReplies($ticket);
